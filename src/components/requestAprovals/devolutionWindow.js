...
export default function VirtualizedList() {
    ...
    const finishStatus = async () => {
        ...
        const list = currentData.equip_list;
        const list_json = JSON.stringify(list)
        await api.put(`/equipment-requests/put-equipment-devolution/${currentData._id}`, {
            equip_list: list_json,
            request_date: currentData.request_date,
            user_id: currentData.user_id,
    ...
    async function GetItens() {
        const data = await api.get(`/equipment-requests/devolution/approvals`).then((response) => {
